 ...
 *
 * Kilo's VS Code webviews use the `babel-code.new.fontSize` setting, not
...
describe("webview font-size architecture", () => {
  ...
  it("injects and live-broadcasts the webview font-size setting to all webview providers", () => {
    ...
    expect(
      missing.map(rel),
      `Webview providers that stay open must broadcast fontSizeChanged when babel-code.new.fontSize changes.\n` +
