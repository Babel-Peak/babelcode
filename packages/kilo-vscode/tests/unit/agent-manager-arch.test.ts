...
describe("Agent Manager Worktree Actions", () => {
  ...
  it("registers distinct dialog and quick-create worktree shortcuts", () => {
    ...
    const dialog = manifest.contributes.keybindings.find(
      (item) => item.command === "babel-code.new.agentManager.newWorktree",
    )
    const quick = manifest.contributes.keybindings.find(
      (item) => item.command === "babel-code.new.agentManager.quickWorktree",
    )
  ...
  it("reserves Cmd+Shift+M for the Agent Manager instead of Problems", () => {
    ...
    const manager = manifest.contributes.keybindings.find((item) => item.command === "babel-code.new.agentManagerOpen")
  ...
  it("forwards the quick-worktree command to immediate creation", () => {
    ...
    const start = source.indexOf('vscode.commands.registerCommand("babel-code.new.agentManager.quickWorktree"')
    const end = source.indexOf('vscode.commands.registerCommand("babel-code.new.agentManager.openWorktree"', start)
