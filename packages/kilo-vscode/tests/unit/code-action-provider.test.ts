  ...
  describe("provideCodeActions", () => {
    ...
    describe("non-empty range, no diagnostics", () => {
      it("returns Add, Explain, Improve actions", () => {
        ...
        expect(titles).toContain("Add to Babel Code")
        expect(titles).toContain("Explain with Babel Code")
        expect(titles).toContain("Improve with Babel Code")
      })

      it("does not include Fix action", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(0) as never)
        expect(result.map((a) => a.title)).not.toContain("Fix with Babel Code")
      })
      ...
      it("uses correct command IDs", () => {
        ...
        expect(commands).toContain("babel-code.new.addToContext")
        expect(commands).toContain("babel-code.new.explainCode")
        expect(commands).toContain("babel-code.new.improveCode")
      })
    ...
    describe("non-empty range, with diagnostics", () => {
      it("returns Add and Fix actions", () => {
        ...
        const titles = result.map((a) => a.title)
        expect(titles).toContain("Add to Babel Code")
        expect(titles).toContain("Fix with Babel Code")
      })
      ...
      it("does not include Explain or Improve actions", () => {
        ...
        const titles = result.map((a) => a.title)
        expect(titles).not.toContain("Explain with Babel Code")
        expect(titles).not.toContain("Improve with Babel Code")
      })
      ...
      it("Fix action is preferred", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(1) as never)
        const fix = result.find((a) => a.title === "Fix with Babel Code")
      ...
      it("uses correct Fix command ID", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(1) as never)
        const fix = result.find((a) => a.title === "Fix with Babel Code")
        expect(fix?.command?.command).toBe("babel-code.new.fixCode")
      })
    })
  })
})
