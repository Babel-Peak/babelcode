...
export function registerTerminalActions(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("babel-code.new.terminalAddToContext", async (args: any) => {
    ...
    vscode.commands.registerCommand("babel-code.new.terminalFixCommand", async (args: any) => {
      let content = args?.selection as string | undefined
      if (!content) {
        content = (await getTerminalContents(1)).content
      }
    ...
    vscode.commands.registerCommand("babel-code.new.terminalExplainCommand", async (args: any) => {
      let content = args?.selection as string | undefined
      if (!content) {
        content = (await getTerminalContents(1)).content
      }
