...
const AM_PREFIX = "babel-code.new.agentManager."

/** Global commands whose keybindings are forwarded to the webview. */
const GLOBAL_KEYBINDINGS: Record<string, string> = {
  "babel-code.new.agentManagerOpen": "agentManagerOpen",
  "babel-code.new.cycleAgentMode": "cycleAgentMode",
  "babel-code.new.cyclePreviousAgentMode": "cyclePreviousAgentMode",
}

function addBinding(bindings: Record<string, string>, name: string, value: string, when?: string): void {
  if (name === "newTerminalTab" && when?.includes("!babel-code.new.agentManagerSideTerminalFocused")) {
